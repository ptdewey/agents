/**
 * Sourced from https://github.com/mitsuhiko/agent-stuff under Apache License V2.0
 *
 * ----
 *
 ...
 * - Review jj changes, revsets, and arbitrary "since <rev>" ranges
 * - Review a GitHub pull request in Git repos (checks out the PR locally)
 * - Review against a base branch (Git PR style)
 * - Review uncommitted Git changes
 * - Review a specific Git commit or jj revision
 * - Shared custom review instructions (applied to all review modes when configured)
 *
 * Usage:
 * - `/review` - show interactive selector
 * - `/review current` - review current jj change (@)
 * - `/review previous` - review previous jj change (@-)
 * - `/review since main` - review jj changes from main to @
 * - `/review since abc123 @` - review jj changes from abc123 to @
 * - `/review revset 'main..@'` - review an arbitrary jj revset
 * - `/review pr 123` - review GitHub PR #123 in Git repos (checks out locally)
 * - `/review pr https://github.com/owner/repo/pull/123` - review GitHub PR from URL
 * - `/review uncommitted` - review uncommitted Git changes directly
 * - `/review branch main` - review against main branch
 * - `/review commit abc123` - review specific Git commit or jj revision
...
// Review target types (matching Codex's approach, plus jj-first workflows)
type ReviewTarget =
  | { type: "uncommitted" }
  ...
  | { type: "jjCurrent" }
  | { type: "jjPrevious" }
  | { type: "jjSince"; base: string; to?: string }
  | { type: "jjRevset"; revset: string }
...
const UNCOMMITTED_PROMPT =
  "Review the current Git working-tree changes (staged, unstaged, and untracked files) and provide prioritized findings.";

const GIT_LOCAL_CHANGES_REVIEW_INSTRUCTIONS =
  "Also include local Git working-tree changes (staged, unstaged, and untracked files) from this branch. Use `git status --porcelain`, `git diff`, `git diff --staged`, and `git ls-files --others --exclude-standard` so local fixes are part of this review cycle.";

const JJ_LOCAL_CHANGES_REVIEW_INSTRUCTIONS =
  "Also include the current jj working-copy change. Use `jj st` and `jj diff -r @ --git` so local fixes are part of this review cycle. Do not use git porcelain commands in this jj repo.";

const JJ_CURRENT_PROMPT =
  "Review the current jj working-copy change (`@`). Use `jj st`, `jj log -r @ --no-graph`, and `jj diff -r @ --git` to inspect the change. Do not use git porcelain commands. Provide prioritized, actionable findings.";

const JJ_PREVIOUS_PROMPT =
  "Review the previous jj change (`@-`). Use `jj log -r @- --no-graph` and `jj diff -r @- --git` to inspect the change. Do not use git porcelain commands. Provide prioritized, actionable findings.";
...
const JJ_BASE_REV_PROMPT =
  "Review the code changes from jj base rev/bookmark `{base}` to `{to}`. Use `jj diff --from {baseShell} --to {toShell} --git` to inspect the net diff and `jj log -r {rangeShell}` to understand the included changes. Do not use git porcelain commands. Provide prioritized, actionable findings.";

const COMMIT_PROMPT_WITH_TITLE =
  'Review the code changes introduced by Git commit {sha} ("{title}"). Provide prioritized, actionable findings.';

const COMMIT_PROMPT =
  "Review the code changes introduced by Git commit {sha}. Provide prioritized, actionable findings.";

const JJ_REVISION_PROMPT_WITH_TITLE =
  'Review the code changes introduced by jj revision {sha} ("{title}"). Use `jj log -r {shaShell} --no-graph` and `jj diff -r {shaShell} --git`. Do not use git porcelain commands. Provide prioritized, actionable findings.';

const JJ_REVISION_PROMPT =
  "Review the code changes introduced by jj revision {sha}. Use `jj log -r {shaShell} --no-graph` and `jj diff -r {shaShell} --git`. Do not use git porcelain commands. Provide prioritized, actionable findings.";

const JJ_REVSET_PROMPT =
  "Review the changes represented by jj revset `{revset}`. Use `jj log -r {revsetShell}` and `jj diff -r {revsetShell} --git`. If `jj diff -r` reports that the revset is non-contiguous or unsupported, use `jj log -r {revsetShell}` to list the revisions and review each relevant change with `jj diff -r <revision> --git`. Do not use git porcelain commands. Provide prioritized, actionable findings.";
...
async function getVcsKind(pi: ExtensionAPI): Promise<VcsKind> {
  const { code: jjCode } = await pi.exec("jj", ["root"]);
  if (jjCode === 0) {
    return "jj";
  }
...
async function hasJjCurrentChange(pi: ExtensionAPI): Promise<boolean> {
...
async function buildReviewPrompt(
  pi: ExtensionAPI,
  target: ReviewTarget,
  options?: { includeLocalChanges?: boolean; vcsKind?: VcsKind },
): Promise<string> {
  const includeLocalChanges = options?.includeLocalChanges === true;
  const vcsKind = options?.vcsKind ?? "git";
  ...
  switch (target.type) {
    ...
    case "jjCurrent":
      return JJ_CURRENT_PROMPT;

    case "jjPrevious":
      return JJ_PREVIOUS_PROMPT;

    case "jjSince": {
      const to = target.to?.trim() || "@";
      const basePrompt = JJ_BASE_REV_PROMPT.replace(/{base}/g, target.base)
        .replace(/{to}/g, to)
        .replace(/{baseShell}/g, shellQuote(target.base))
        .replace(/{toShell}/g, shellQuote(to))
        .replace(/{rangeShell}/g, shellQuote(`${target.base}..${to}`));
      return includeLocalChanges
        ? `${basePrompt} ${JJ_LOCAL_CHANGES_REVIEW_INSTRUCTIONS}`
        : basePrompt;
    }

    case "jjRevset": {
      const basePrompt = JJ_REVSET_PROMPT.replace(
        /{revset}/g,
        target.revset,
      ).replace(/{revsetShell}/g, shellQuote(target.revset));
      return includeLocalChanges
        ? `${basePrompt} ${JJ_LOCAL_CHANGES_REVIEW_INSTRUCTIONS}`
        : basePrompt;
    }
    ...
    case "baseBranch": {
      if (vcsKind === "jj") {
        return buildReviewPrompt(
          pi,
          { type: "jjSince", base: target.branch },
          options,
        );
      }
      ...
      return includeLocalChanges
        ? `${basePrompt} ${GIT_LOCAL_CHANGES_REVIEW_INSTRUCTIONS}`
        : basePrompt;
    }

    case "commit": {
      if (vcsKind === "jj") {
        const shaShell = shellQuote(target.sha);
        if (target.title) {
          return JJ_REVISION_PROMPT_WITH_TITLE.replace(/{sha}/g, target.sha)
            .replace(/{shaShell}/g, shaShell)
            .replace("{title}", target.title);
        }
        return JJ_REVISION_PROMPT.replace(/{sha}/g, target.sha).replace(
          /{shaShell}/g,
          shaShell,
        );
      }
...
function getUserFacingHint(target: ReviewTarget): string {
  switch (target.type) {
    ...
    case "jjCurrent":
      return "current jj change (@)";
    case "jjPrevious":
      return "previous jj change (@-)";
    case "jjSince": {
      const to = target.to?.trim() || "@";
      return `jj changes from '${target.base}' to '${to}'`;
    }
    case "jjRevset":
    ...
    case "commit": {
      ...
      return target.title
        ? `revision ${shortSha}: ${target.title}`
        : `revision ${shortSha}`;
    }
...
const GIT_REVIEW_PRESETS = [
  ...
  { value: "commit", label: "Review a commit", description: "(Git)" },
  {
    value: "pullRequest",
...
const JJ_REVIEW_PRESETS = [
  {
    value: "jjCurrent",
    label: "Review current change (@)",
    description: "(jj)",
  },
  {
    value: "jjPrevious",
    label: "Review previous change (@-)",
    description: "(jj)",
  },
  {
    value: "jjSince",
    label: "Review since base rev/bookmark",
    description: "(jj diff --from base --to @)",
  },
  {
    value: "jjRevset",
    label: "Review arbitrary revset",
    description: "(jj)",
  },
  {
    value: "folder",
...
const REVIEW_PRESETS = [...GIT_REVIEW_PRESETS, ...JJ_REVIEW_PRESETS] as const;
...
export default function reviewExtension(pi: ExtensionAPI) {
  ...
  /**
   * Determine the smart default review type based on repository state
   */
  async function getSmartDefault(vcsKind: VcsKind): Promise<ReviewPresetValue> {
    if (vcsKind === "jj") {
      return (await hasJjCurrentChange(pi)) ? "jjCurrent" : "jjPrevious";
    }
  ...
  async function showReviewSelector(
    ...
    const vcsKind = await getVcsKind(pi);
    const presets = vcsKind === "jj" ? JJ_REVIEW_PRESETS : GIT_REVIEW_PRESETS;

    // Determine smart default (but keep the list order stable)
    const smartDefault = await getSmartDefault(vcsKind);
    const presetItems: SelectItem[] = presets.map((preset) => ({
      value: preset.value,
      label: preset.label,
    ...
    while (true) {
      ...
      switch (result) {
        ...
        case "jjCurrent":
          return { type: "jjCurrent" };

        case "jjPrevious":
          return { type: "jjPrevious" };

        case "jjSince": {
          const target = await showJjSinceInput(ctx);
          if (target) return target;
          break;
        }

        case "jjRevset": {
          const target = await showJjRevsetInput(ctx);
          if (target) return target;
          break;
        }
  ...
  async function showJjSinceInput(
    ctx: ExtensionContext,
  ): Promise<ReviewTarget | null> {
    const result = await ctx.ui.editor(
      "Enter jj base rev/bookmark to review since (optionally: <base> <to>):",
      "main",
    );

    if (!result?.trim()) return null;
    const parts = tokenizeArgs(result.trim());
    const base = parts[0]?.trim();
    if (!base) return null;
    return { type: "jjSince", base, to: parts[1]?.trim() || undefined };
  }

  async function showJjRevsetInput(
    ctx: ExtensionContext,
  ): Promise<ReviewTarget | null> {
    const result = await ctx.ui.editor(
      "Enter jj revset to review:",
      "@-",
    );

    if (!result?.trim()) return null;
    return { type: "jjRevset", revset: result.trim() };
  }
  ...
  function parseArgs(args: string | undefined): ParsedReviewArgs {
    ...
    switch (subcommand) {
      ...
      case "current":
      case "@":
        return { target: { type: "jjCurrent" }, extraInstruction };

      case "previous":
      case "prev":
      case "parent":
      case "@-":
        return { target: { type: "jjPrevious" }, extraInstruction };

      case "since":
      case "base": {
        const base = parts[1];
        if (!base) return { target: null, extraInstruction };
        return {
          target: { type: "jjSince", base, to: parts[2] || undefined },
          extraInstruction,
        };
      }
      ...
      case "range": {
        ...
        const to = parts[2];
        if (!base) return { target: null, extraInstruction };
        return { target: { type: "jjSince", base, to }, extraInstruction };
      }

      case "revset":
      case "revs": {
        const revset = parts.slice(1).join(" ").trim();
        if (!revset) return { target: null, extraInstruction };
        return { target: { type: "jjRevset", revset }, extraInstruction };
      }

      case "change":
      case "revision":
      case "rev": {
        const revset = parts[1];
        if (!revset) return { target: null, extraInstruction };
        return { target: { type: "jjRevset", revset }, extraInstruction };
      }
  ...
  pi.registerCommand("review", {
    description:
      "Review code changes (jj revsets/ranges, PR, uncommitted, branch, commit, or folder)",
    handler: async (args, ctx) => {
      ...
      // Check if we're in a supported repository. Prefer jj semantics when both are present.
      const vcsKind = await getVcsKind(pi);
      if (vcsKind === "none") {
        ctx.ui.notify("Not a jj or Git repository", "error");
        return;
      }
      ...
      if (parsed.target) {
        if (parsed.target.type === "pr") {
          if (vcsKind === "jj") {
            ctx.ui.notify(
              "GitHub PR checkout is hidden in jj repos. Review the local jj state instead (for example `/review since main` or `/review revset 'main..@'`).",
              "warning",
            );
          } else {
